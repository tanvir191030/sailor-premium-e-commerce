...
const SizeChartModal = ({ open, onClose }: Props) => (
  <AnimatePresence>
    {open && (
      <>
        <motion.div
        ...
        >
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <h2 className="font-serif text-lg text-foreground">সাইজ চার্ট</h2>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {measurements.map((row, i) => (
                      <tr
                        key={row.size}
                        className={`border-t border-border transition-colors ${i % 2 === 0 ? "" : "bg-secondary/30"}`}
                      >
