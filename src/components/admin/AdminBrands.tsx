...
const AdminBrands = () => {
  ...
  const { data: products = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("products").select("id, brand");
      if (error) throw error;
      return data;
    },
  });
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "ব্র্যান্ড যোগ হয়েছে" });
      setNewName("");
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card p-5 rounded-xl shadow-sm border border-border">
        <h3 className="font-serif text-lg text-foreground mb-4">Add New Brand</h3>
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
        <div className="p-4 border-b border-border">
          <h3 className="font-medium text-foreground flex items-center gap-2">
            <Award size={18} /> All Brands ({brands.length})
          </h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {brands.length === 0 && (
                <tr><td colSpan={4} className="p-8 text-center text-muted-foreground">No brands yet. Add one above.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-card rounded-xl shadow-2xl w-full max-w-sm p-6 border border-border text-center">
            ...
            {(productCountMap[deleteTarget.name] || 0) > 0 && (
              <p className="text-xs text-destructive mb-2">⚠️ এই ব্র্যান্ডে {productCountMap[deleteTarget.name]}টি প্রোডাক্ট আছে</p>
            )}
